...
struct AddressHasher {
    ...
    size_t operator() (const Address& address) const {
        const size_t a = strHash(address.city);
        const size_t b = strHash(address.street);
        const size_t c = intHash(address.building);
...
struct PersonHasher {
    ...
    size_t operator() (const Person& person) const {
        const size_t a = strHash(person.name);
        const size_t b = intHash(person.height);
        const size_t c = doubleHash(person.weight);
        const size_t d = addressHash(person.address);
